#ifndef PHYPP_GENERIC_HPP
#define PHYPP_GENERIC_HPP
...
namespace impl {
    ...
    struct has_right_dim {
        template <typename T>
        struct type : bool_constant<elem_dim<T>::value == D || elem_dim<T>::value == 0> {};
    };
    ...
    struct vectorize_lambda_t {
        ...
        void run(std::false_type, std::true_type, Args&& ... args) {
            ...
            static_assert(are_all_true<bool_list<has_right_dim<D>::template type<Args>::value...>>::value,
