...
namespace vec_ilist {
    ...
    struct helper {
        ...
        static void fill_(vec<Dim,Type>& v, meta::nested_initializer_list<1,dtype> il, uint_t& idx, meta::cte_t<Dim-1>) {
            vif_check(il.size() == v.dims[Dim-1], "heterogeneous intializer lists are not allowed");
            for (auto& t : il) {
                v.data[idx] = t;
                ++idx;
            }
        ...
        static void fill_(vec<Dim,Type>& v, meta::nested_initializer_list<Dim-N,dtype> il, uint_t& idx, meta::cte_t<N>) {
            vif_check(il.size() == v.dims[N], "heterogeneous intializer lists are not allowed");
            for (auto& t : il) {
