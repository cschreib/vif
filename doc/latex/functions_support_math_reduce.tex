...
The functions \cppinline{partial_min()} and \cppinline{partial_max()} will apply respectively \cppinline{min()} and \cppinline{min()} on the \cppinline{d}th dimension of the vector (zero being the first dimension) and reduce its number of dimensions by one.
