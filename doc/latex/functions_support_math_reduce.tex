\funcitem \cppinline|void run_dim_idx(uint_t d, F f, ...)| \itt{run_dim_idx}

\funcitem \cppinline|vec<D-1,T> run_dim(vec<D,T> v, uint_t d, F f)| \itt{run_dim}

\funcitem \cppinline|T total(vec<D,T> v)| \itt{total}
...
\funcitem \cppinline|uint_t fraction_of(vec<D,bool> v)| \itt{fraction_of}
...
\cppinline|vec2u histogram2d(vec x, y, vec<2,U> bx, by, F func)|

\funcitem \cppinline|vec<D,bool> sigma_clip(vec<D,T> v, double pl, double pu)| \itt{sigma_clip}

\funcitem \cppinline|vec<D,bool> mad_clip(vec<D,T> v, double s)| \itt{mad_clip}
