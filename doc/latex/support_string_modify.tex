...
\funcitem \cppinline|string regex_replace(string s, reg, rep)| \itt{regex_replace}

\cppinline|string regex_replace(string s, reg, rep, T f)|

This function will search in the string \cppinline{s} using the regular expression \cppinline{reg} (see \cppinline{regex_match}) to locate some expressions. These expressions are then modified according to the pattern given in \cppinline{rep}, and will be inserted back in a copy of \cppinline{s}, which is eventually returned. The expression to replace can contain sub-expressions that will be reused in the replacement. To do so, the sub-expressions must be enclosed in parentheses in \cppinline{reg}, and referred to as \cppinline{\1}, \cppinline{\2}, etc., in \cppinline{rep}.

There are little rules about the replacement pattern. It can be of arbitrary length, and can refer to a given sub-expression (e.g., \cppinline{\1}) any number of times (including zero). These sub-expressions can appear in any order. Apart from the sub-expression identifiers, there is no special character. As for the regular expression, it is advised to use the \emph{raw string literal} notation (see \cppinline{regex_match}).
...
// So we have two sub-expressions, the first (\1) is "X", the second
// (\2) is "Y".

// In the replacement pattern, we just specify how we want to change
// the numbers that were matched. Here we say that we want:
// '\1' : the first sub-expression
// '='  : followed by an equal sign (indeed, it was not captured)
// '"'  : a double quote
...
The second overload will mostly do the same thing. The only difference is that, before inserting back a matched sub-expression in the replacement pattern, the sub-expression is fed to the user-supplied function \cppinline{f}, which can do any kind of operation on this expression and return the modified version. The function \cppinline{f} must take two arguments: first, an integer specifying the ID of the sub-expression that was matched (starting from \cppinline{1}); second, a string containing the sub-expression itself.

\begin{example}
\begin{cppcode}
// We take the same example as above, but this time we also want to
// change "X" to upper case increment "Y" by one. This cannot
// be achieved with the first overload.

// The function parameters are unchanged (see above), but we add
// a fourth argument
std::string n = regex_replace(
    s,                    // the string to analyze
    R"(([a-z])=([0-9]))", // the regular expression
    R"(\1="\2")",         // the replacement pattern
    [](uint_t i, std::string e) {
        // i: index of the sub-expression
        // e: the sub-expression
