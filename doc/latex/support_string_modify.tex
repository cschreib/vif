...
This function will perform regular expression matching (see \cppinline{regex_match}), and will return a vector containing all the extracted substrings. To extract one or more substrings in the regular expression, just enclose the associated patterns in parentheses. The returned vector is two dimensional: the first dimension correspond to the number of times the whole regular exception was matched in the provided string, the second dimension corresponds to each extracted substring.
...
This function will perform regular expression matching (see \cppinline{regex_match}) using the regex \cppinline{reg}, to locate some expressions in the string \cppinline{s} and replace them with another expression \cppinline{rep}. The expression to replace can contain sub-expressions that will be reused in the replacement. To do so, the sub-expressions must be enclosed in parentheses in \cppinline{reg}, and referenced as \cppinline{\1}, \cppinline{\2}, etc., in \cppinline{rep}.

This function is very similar to the \texttt{sed} program.

\begin{example}
\begin{cppcode}
std::string s = "array{5,6.25,7,28}end45.6ddfk";
// Find all floating point numbers, and swap their
// integer and fractional parts.
std::string n = regex_replace(
    s,                       // the string to analyse
    R"(([0-9]+)\.([0-9]+))", // the regular expression
    R"(\2.\1)"               // the replacement pattern
);

// Here we have two sub-expressions, the first being the integer
// part of the number (\1), the second being the fractional
// part (\2).

n; // "array{5,25.6,7,28}end6.45ddfk"
