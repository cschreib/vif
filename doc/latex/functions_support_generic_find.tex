...
\funcitem \cppinline|vec1u unique_ids(vec v)| \itt{unique_ids}

\cppinline|vec1u unique_ids(vec v, vec1u sid)|

\cppinline|vec1u unique_ids_sorted(vec v)| \itt{unique_ids_sorted}

This function will traverse the provided vector \cppinline{v} and find all the unique values. It will store the indices of these values (if a value is present more than once inside \cppinline{v}, the index of the first one will be used) and return them inside an index vector. See also \cppinline{unique_values()} which directly returns values instead of indices.

Internally, the function needs to sort \cppinline{v}. To optimize things, the second version of this function takes a second argument \cppinline{id} that contains indices that will sort \cppinline{v} (e.g., \cppinline{id} can be the return value of \cppinline{sort(v)}), so that it does not need to be computed again if you already had to sort \cppinline{v} for some other reason.

Lastly, the third version of this function (named \cppinline{unique_ids_sorted()}) simply assumes that \cppinline{v} is already sorted, which makes it even faster.
...
\funcitem \cppinline|vec1u unique_values(vec v)| \itt{unique_values}

\cppinline|vec1u unique_values(vec v, vec1u sid)|

\cppinline|vec1u unique_values_sorted(vec v)| \itt{unique_values_sorted}

This function will traverse the provided vector \cppinline{v} and find all the unique values. See also \cppinline{unique_ids()} which instead returns indices to the unique values, instead of the values themselves.

Internally, the function needs to sort \cppinline{v}. To optimize things, the second version of this function takes a second argument \cppinline{id} that contains indices that will sort \cppinline{v} (e.g., \cppinline{id} can be the return value of \cppinline{sort(v)}), so that it does not need to be computed again if you already had to sort \cppinline{v} for some other reason.

Lastly, the third version of this function (named \cppinline{unique_values_sorted()}) simply assumes that \cppinline{v} is already sorted, which makes it even faster.
